...
DELIMITER //
CREATE function
  fn_total_ventas (p_id_vendedores INT)
  RETURNOS FLOAT
  NO SQL
BEGIN
...
SET resultado =
  (
         SELECT sum(f.cant_venta*p.precio_pub) total
         FROM   facturas f
...
DELIMITER //
CREATE function
  fn_promedio_categoria (p_categoria_exposicion VARCHAR(20))
  RETURNS FLOAT
  NO SQL
BEGIN
  DECLARE resultado FLOAT;
